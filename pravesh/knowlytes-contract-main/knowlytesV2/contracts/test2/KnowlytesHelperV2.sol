...
// import "hardhat/console.sol";
 ...
 contract KnowlytesHelperV2 is StorageV2,IHelper{
    ...
    function isFreeListedAddress(bytes32[] calldata proof) external view {
        require(isFreeMintingPaused, "Free Mint Paused");
        require(isFreeValid(proof,keccak256(abi.encodePacked(msg.sender))),"Not FreeListed Address");
    }
    ...
    function isFreeValid(bytes32[] calldata proof, bytes32 leaf) private view returns (bool) {
        return MerkleProof.verify(proof, StorageV2.freeRootHash, leaf);
    }

    function freezeValues(uint256[][6] calldata min, uint256[][6] calldata totalId) external{
        require(msg.sender == StorageV2.manager,"Only Manager");
        for(uint256 i =0; i<min[0].length; ++i){
            StorageV2.hatFreezedMap[min[0][i]] = true;
        }
        for(uint256 i =0; i<min[1].length; ++i){
            StorageV2.jacketFreezedMap[min[1][i]] = true;
        }
        for(uint256 i =0; i<min[2].length; ++i){
            StorageV2.hairFreezedMap[min[2][i]] = true;
        }
        for(uint256 i =0; i<min[3].length; ++i){
            StorageV2.noseFreezedMap[min[3][i]] = true;
        }
        for(uint256 i =0; i<min[4].length; ++i){
            StorageV2.glassFreezedMap[min[4][i]] = true;
        }
        for(uint256 i =0; i<min[5].length; ++i){
    ...
    function updateBatchTraitType(uint256[] calldata tId, uint256[] calldata _type, uint256[] calldata _value) external {
        require(msg.sender == StorageV2.manager, "Only Manager can Call");
        for(uint256 i=0; i<tId.length; ++i){
            delete StorageV2.nftDetails[tId[i]].haveChanged;
            delete StorageV2.nftDetails[tId[i]].hashedProof;
            if (_type[i] == 1) {
    ...
    function setTraitvalues(uint256 i, uint256 key, string calldata value) external {
        if (i == 1) {
            hatMap[key] = value;
            sHat = sHat+1;
        }
    ...
    //TRYING
    function updateNFTDetail(uint256 tokenId) external {
        StorageV2.TokenMetaData storage nft = StorageV2.nftDetails[tokenId];
        nft.tokenId = tokenId;
